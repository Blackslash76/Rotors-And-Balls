...
public class CreatureBehavior : MonoBehaviour
{
	...
	void OnGUI()
    {
        Vector3 topOfEnemy = Camera.main.WorldToScreenPoint(transform.position + new Vector3(0, 0, transform.localScale.z / 2));
    ...
    void OnTriggerEnter(Collider other_obj)
    {
        ...
        else if (other_obj.transform.name.Contains("ArrowTemplate"))
        {
            ...
            other_obj.GetComponent<Rigidbody>().velocity = -other_obj.GetComponent<Rigidbody>().mass * other_obj.GetComponent<Rigidbody>().velocity / 10;
        }
    }
