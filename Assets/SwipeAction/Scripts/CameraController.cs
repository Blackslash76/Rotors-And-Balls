	...
	void Update ()
    {
        if (Camera.main.orthographic)
        {
            ...
            if (Physics.Raycast(ray, out target))
            {
                ...
                if (target.transform.name.Contains("house_roof") && target.transform.GetComponent<Renderer>().material.color.a != 0)
                {
                    target.transform.GetComponent<Renderer>().material.color = new Color(1, 1, 1, 0);
                }

                if (previous_transform.name == "house_roof" && current_transform.name != "house_roof")
                {
                    previous_transform.GetComponent<Renderer>().material.color = new Color(1, 1, 1, 1);
                }
            }
        }
        else
